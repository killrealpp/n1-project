...
- translation model: `OPENROUTER_TRANSLATION_MODEL=openai/gpt-4.1-mini`;
- article model: `OPENROUTER_ARTICLE_MODEL=openai/gpt-4.1`.
...
Dzen articles use a separate editorial prompt. The model receives the latest candidate translated posts, selects only a coherent semantic cluster, writes a source-grounded Russian article, and sends it to Telegram admin review before Dzen bridge publishing on weekdays.
